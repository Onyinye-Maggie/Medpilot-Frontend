...
export default function RefillsPage() {
  ...
  const medOptions = activeMeds.map(m => ({
    value: m._id || m.id,
    label: `${m.name} — ${m.dosage} (${m.remainingQuantity ?? m.quantity ?? 0} left)`,
  }));
  ...
  const filtered = refills.filter(r => {
    ...
    return (r.medication?.name || '').toLowerCase().includes(q) ||
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (v) => { try { return v ? format(parseISO(v), 'dd MMM yyyy') : '—'; } catch { return '—'; } }
    },
    {
    ...
    {
      key: 'isUrgent', label: '',
      render: (v) => v ? <Badge variant="danger">Urgent</Badge> : null
    },
    {
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                {medsNeedingRefill.map(m => (
                  <button key={m._id || m.id} onClick={() => openCreate(m._id || m.id)}
      ...
      {!loading && activeMeds.length === 0 && (
        <Card style={{ marginBottom: '16px', padding: '16px', borderColor: 'rgba(255,165,2,0.3)', background: 'rgba(255,165,2,0.05)' }}>
          <p style={{ color: 'var(--accent-warning)', fontSize: '14px' }}>
            ⚠ Add medications first before requesting refills.
          </p>
        </Card>
      )}
